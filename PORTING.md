...
verify against a live boot dump, not just a green gate).  Almost none of
...
| | Baseball (done) | NFL Football (done) | Auto Racing (done) | Armor Battle (done) | Utopia (done) | Frog Bog (done) | Boxing | NASL Soccer (done) |
|---|---|---|---|---|---|---|---|---|
| timer table | `$501C` | `$5026` | `$5029` | `$5050` | `$501C` | `$501C` | `$501C` | `$501C` |
| start-of-game | `$5034` | `$5075` | `$5037` | `$505A` | `$504C` | `$5991` | `$5053` | `$506A` |
| game entries / interval | `$5048` / 2 | `$5034` / 1, `$56EF` / 15 | `$511E` / 1, `$51B7` / 15 | `$554E` / 1 | `$51A3` / 1 | **`$5D89`/`$5572` / 1, `$5DE3` / 60, `$5DAA` / 600, `$529A` / 15 (5 entries, most of any port)** | `$51F0`,`$5A0F`,`$57AB` / 1, `$52F2`,`$5297` / 16 | `$5042` / 1, `$5AB8` / 2, `$505F` / 5, `$503A` / 20 |
| **tick rate** | **10 Hz (100 ms)** | **20 Hz (50 ms)** | **20 Hz (50 ms)** | **20 Hz (50 ms)** | **20 Hz (50 ms)** | **20 Hz (50 ms)** | **20 Hz (50 ms)** | **MEASURED 14.97 Hz (66.8 ms) — header implies 20 Hz; see §7.28** |
| RNG sites to wrap | 5 | 3 | 4 | **9 (+1 stir left volatile)** | 15 (all X_RAND2) | 9 (6× X_RAND1 + 3× X_RAND2 — 2nd port needing both) | 8 | 5 (3× X_RAND1 + 2× X_RAND2 — 3rd port needing both) |
| `$035D` phase installs | 7 | 1 | 1 | 4 (3 tables, one EXEC-resident: `$1906`) | 2 (`$5125`, `$1906`) + the EXEC number entry's own | 2 (`$55A6` live table, `$1906` EXEC null — both installed once, tail of `.START`) | 3 | **1 (`$5869`, installed once at `$50A1`, NEVER changed — no phase readable off the table at all)** |
| `$011F`/`$0120` reads found | 2 | 1 | 0 | 1 (both seats via `[$011F+player]`) | **0 (true dispatch-only)** | 1 (both seats via `[$011F+player]`, Armor Battle's hybrid shape) | 0 | **4 (2 walking-pointer setups + 1 direct pair — most of any port)** |
| timer arm/stop calls | 1 (`$181E`) | 0 | 0 | **3** (`$1838`/`$1844`, stale absolute entry addr → shims) | 0 | **6** (across 3 entries — largest shim surface of any port) | 5 (`$1838`/`$1844`) | **0** |
| STIC writes | **0** | 3 (incl. `$0030`) | **7** (incl. `$0030`/`$0031`) | **0** (recon's 1 hit = class-1 false positive) | **0** | **0** (2 recon hits, both class-1 `SDBD`-prefix false positives) | 1 (`$0020`) | 2 (`$0030` scroll + `$0020`) |
| display mode (`$500E`) | 0 colour stack | 0 colour stack | **1 fg/bg** | 0 colour stack | 0 colour stack | **1 fg/bg** | 0 colour stack | 0 colour stack |
| main loop | EXEC | EXEC | EXEC | **cart clone during battle (§7.15)** | EXEC | EXEC | EXEC (verify!) | EXEC — **but entry 1 calls `X_SCAN` itself (§7.26)** |
