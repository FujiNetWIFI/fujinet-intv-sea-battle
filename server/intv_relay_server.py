#!/usr/bin/env python3
"""Intellivision netplay matchmaking + relay server (NASL Soccer instance).
...
SOCCER IS A STRICTLY 2-PLAYER GAME -- two teams, no third seat.  MAXPLAYERS
is 2 here; the seat plumbing stays general so this file remains diffable
against its siblings, but a room never exceeds two members.
...
PROTO_VER = 2
DEFAULT_DELAY = 3
# Room capacity.  Soccer has two teams and therefore exactly two seats.
MAX_SEATS = 2
...
# name slots), so the roster stays 4-padded even though seats 2 and 3 can
# never be filled.  Shrinking it here silently misframes every client.
ROSTER_SLOTS = 4
...
class LobbyPublisher:
    ...
    def __init__(self, base_url, serverurl, client_url, appkey, region="us",
                 game_name="NASL Soccer", server_name="Soccer Netplay"):
...
def main():
    ...
    ap.add_argument("--game-name", default="NASL Soccer",
                    help="game name for the FujiNet Lobby registration")
    ap.add_argument("--server-name", default="Soccer Netplay",
    ...
    ap.add_argument("--lobby-serverurl",
                    default="TCP://fujinet.online:9109/",
    ...
    ap.add_argument("--lobby-appkey", type=int, default=17,
                    help="FujiNet-registry appkey id for this game "
                         "(NASL Soccer = 17; the Lobby rejects 0)")
    args = ap.parse_args()
    logging.basicConfig(
