...
TRACE_RANGES:
        ...
        ; Virtualized timer state.  Soccer has no arm flags (zero timer-API
        ; sites), so only SC_CNT2/3/4 carry anything -- but the ranges keep
        ; Bowling's exact spans, covering the always-zero RS_SPARE cells
        ; too, so that these bounds, LS_CKSUM's and RS_TAILTBL's stay in
        ; agreement without a §7.16 sweep.  Zeros checksum as zeros.
        DECLE   RS_SPARE0, RS_SPARE0+1  ; $8102
        DECLE   RS_SPARE1, RS_SPARE1+1  ; $810F
        DECLE   SC_CNT2, RS_SPARE4+1    ; $8196-$819B (CNT2/3/4 + 3 spares)
        DECLE   RS_SPARE5, RS_SPARE5+1  ; $819F
        DECLE   0, 0
