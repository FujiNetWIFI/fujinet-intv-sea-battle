...
;
; NOTE (Sea Battle, M3): this file is copied from Soccer's tree per
; PORTING.md §4's "copy unchanged" list, with exactly ONE line added to
; RS_REBASE below (a JSR to SB_REBASE_HOOK, defined in src/hook.asm).
; RS_CLAMP_ISR immediately above it establishes that this file was never
; truly 100% cart-agnostic in practice -- its body already IS Soccer-
; specific content (a code-pointer clamp) wearing a generic-sounding name.
; This cart's own hazard (a computed-jump-index clamp, plus restoring a
; countdown that lives outside the standard image range) doesn't fit
; inside an existing hook, so it gets an explicit one-line call instead of
; being smuggled into RS_CLAMP_ISR's body under a misleading name.
;
...
;   0..146    $015D-$01EF game scratch (bytes; the whole Soccer game state
;             -- scores, match clock, possession, phase, AND the scroll
;             block $015D-$0166: velocity, accumulator, row deltas, the
;             dance handshake and the SAVED EXEC ISR VECTOR at $0160/$0161.
;             That last pair is a code pointer the cart's ISR body copies
;             straight into $0100/$0101, so RS_REBASE clamps it to the EXEC
;             default -- a corrupted wire byte there would install an
;             arbitrary interrupt handler, past every bounds check, because
;             the POSITION is legal.  PORTING.md §7.2's class.)
;   147..626  $0200-$02EF BACKTAB (240 words, LE byte pairs -- the scrolled
;             pitch and the scoreboard travel for free)
;   627..754  $031D-$035C object table (64 words, LE byte pairs; the six
;             players and the ball.  Soccer keeps no cart globals above the
;             stack base -- M2 census)
;   755..768  RNG_LO/HI, two spares, SC_CNT2/3/4 + three spares (the block
;             $8196-$819B), one spare, SC_PASSLEN ($0103 -- not written by
;             this cart, kept as free insurance), GAME_TBL_LO/HI.
;             The spares are always zero: keeping Bowling's exact 14-entry
;             tail is what lets every IMG_* bound below stand unswept.
...
; sweep of every numeric bound in the applier and pusher.  Swept for the
; 761 -> 769 growth: the applier's quick guard is pos_hi < 4 [loose, wrap
; prevention only] and the exact bound is symbolic IMG_TOTAL; the pusher
; is symbolic throughout; SES_FLEN's STATE max stays 99.)
...
@@rb_hd:
    ENDI
        ...
        JSR     R5,     RS_CLAMP_ISR
        JSR     R5,     SB_REBASE_HOOK   ; cart-specific (src/hook.asm):
                                        ;  restores SB_TICK2's real
                                        ;  countdown, clamps $0164
        MVI     RS_R_LO, R0
        MVO     R0,     TICK_LO
        MVI     RS_R_HI, R0
        MVO     R0,     TICK_HI
...
; cart header.
;
; Soccer is a SCROLLING cart, so this is Auto Racing's form, not Baseball's
; -- and it is already correct as inherited; only this comment changed.
;
; The pitch is wider than the screen: L_55B6 accumulates a scroll position
; in $015D-$015F every tick and its ISR body writes $015F to the STIC
; horizontal delay $0030.  Those cells are live sim state inside the CRC
; range and image section 1, so zeroing $0030/$0031 here -- as the Baseball
; engine did -- would fight the game every tick.  What IS static is the
; border extension ($0032 from $500D = $01) and the colour stack + border
; ($0028-$002C from $500F-$5013, all $04 = the pitch green).  After a
; resync the cart's own dance repaints the scroll register from the
; restored cells within one tick.
;
; Terminal/session screens do NOT use this routine -- see
; UI_DISPLAY_NORMALIZE, which can kill the scroll and black the stack
; because the sim is dead there and nothing repaints it.
; ---------------------------------------------------------------------------
RS_DISPLAY_RESET:
        PSHR    R5
        MVI     $500D,  R0
...
; ---------------------------------------------------------------------------
; 14 entries, deliberately the same count and the same cells Bowling used:
; only SC_CNT2/3/4 carry state here, and the RS_SPARE* cells are always
; zero, but keeping the layout fixed means IMG_S3/IMG_TOTAL and every
; bounds guard stay exactly as proven (PORTING.md §7.16).
RS_TAILTBL:
        DECLE   RNG_LO, RNG_HI
        DECLE   RS_SPARE0, RS_SPARE1
        DECLE   SC_CNT2, SC_CNT3, SC_CNT4       ; $8196-$8198
        DECLE   RS_SPARE2, RS_SPARE3, RS_SPARE4 ; $8199-$819B
        DECLE   RS_SPARE5, SC_PASSLEN
