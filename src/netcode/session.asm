...
@@ses_nojoin:
    ENDI
        ...
        B       @@ses_lobby
@@ses_go:
        ; ---- matched: the seat-ordered roster in bowler colours -----------
        ; Seat n = bowler n+1; ball/scoreboard colours per the manual:
        ; bowler 1 RED, 2 LT GREEN, 3 BLUE, 4 DK GREEN -- all four are
        ; primary in FGBG mode (2/5/1/4), unlike Frog Bog's pink.  The
        ; local seat's row is marked with '>' in yellow.
        JSR     R5,     UI_CLS
        CLRR    R0
        MVII    #STR_TITLE, R1
        JSR     R5,     UI_PRINT
        ...
        MVII    #STR_YOUARE, R1
        JSR     R5,     UI_PRINT
        MVI     NET_SEAT, R0            ; the bowler digit, poked directly
        ADDI    #'1'-32, R0
        SLL     R0,     2
...
STR_TITLE:      STRING  "SOCCER NETPLAY"
                DECLE   0
